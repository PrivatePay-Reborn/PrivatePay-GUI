...
## Copyright (c) 2017, The Sumokoin Project (www.sumokoin.org)
'''
Process managers for privatepayd, privatepay-wallet-cli and privatepay-wallet-rpc
'''
...
class ElectroneumdManager(ProcessManager):
    def __init__(self, resources_path, log_level=0, block_sync_size=10):
        ...
        proc_args = u'%s/bin/privatepayd --log-level %d --block-sync-size %d --rpc-bind-port %d' % (resources_path, log_level, block_sync_size, RPC_DAEMON_PORT)
        ProcessManager.__init__(self, proc_args, "privatepayd")
        self.synced = Event()
        self.stopped = Event()
...
class WalletCliManager(ProcessManager):
    ...
    def __init__(self, resources_path, wallet_file_path, wallet_log_path, restore_wallet=False):
        ...
        if not restore_wallet:
            wallet_args = u'%s/bin/privatepay-wallet-cli --generate-new-wallet=%s --create-address-file --log-file=%s' \
                                                % (resources_path, wallet_file_path, wallet_log_path)
        else:
            wallet_args = u'%s/bin/privatepay-wallet-cli --log-file=%s --restore-deterministic-wallet --restore-height=0 --create-address-file' \
                                                % (resources_path, wallet_log_path)
        ProcessManager.__init__(self, wallet_args, "privatepay-wallet-cli")
        self.ready = Event()
        self.last_error = ""
...
class WalletRPCManager(ProcessManager):
    def __init__(self, resources_path, wallet_file_path, wallet_password, app, log_level=2):
        ...
        wallet_log_path = os.path.join(os.path.dirname(wallet_file_path), "privatepay-wallet-rpc.log")
        resources_path = resources_path.replace(' ' ,'__SPACE_REPLACE__')
        wallet_file_path  = wallet_file_path.replace(' ' ,'__SPACE_REPLACE__')
        wallet_log_path = wallet_log_path.replace(' ' ,'__SPACE_REPLACE__')
        wallet_rpc_args = u'%s/bin/privatepay-wallet-rpc --disable-rpc-login --wallet-file %s --log-file %s --rpc-bind-port %d --log-level %d --daemon-port %d --password %s' % (resources_path, wallet_file_path, wallet_log_path, RPC_DAEMON_PORT+2, log_level, RPC_DAEMON_PORT, wallet_password)

        ProcessManager.__init__(self, wallet_rpc_args, "privatepay-wallet-rpc")
