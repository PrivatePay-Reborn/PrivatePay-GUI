...
class WalletCliManager(ProcessManager):
    ...
    def __init__(self, resources_path, wallet_file_path, wallet_log_path, restore_wallet=False, restore_height=0):
        if not restore_wallet:
            wallet_args = u'%s/bin/privatepay-wallet-cli --daemon-address %s --generate-new-wallet=%s --log-file=%s ' \
                                                % (resources_path, REMOTE_DAEMON_ADDRESS, wallet_file_path, wallet_log_path)
        else:
            wallet_args = u'%s/bin/privatepay-wallet-cli --daemon-address %s --log-file=%s --restore-deterministic-wallet --restore-height %d' \
...
class WalletRPCManager(ProcessManager):
    def __init__(self, resources_path, wallet_file_path, wallet_password, app, log_level=1, enable_ssl=False):
        ...
        if enable_ssl:
            wallet_rpc_args = u'%s/bin/privatepay-wallet-rpc --daemon-address %s --wallet-file %s --log-file %s --rpc-bind-port %d --user-agent %s --log-level %d --enable-ssl --cacerts-path %s' \
            ...
            wallet_rpc_args = u'%s/bin/privatepay-wallet-rpc --daemon-address %s --wallet-file %s --log-file %s --rpc-bind-port %d --user-agent %s --log-level %d' \
